...
class UserFriendshipTest < ActiveSupport::TestCase
  ...
  context "#mutual_friendship" do
    setup do 
      ...
      @friendship1 = users(:christina).user_friendships.where(friend_id: users(:will).id).first
      @friendship2 = users(:will).user_friendships.where(friend_id: users(:christina).id).first
    end
  ...
  context "#accept!" do
    setup do
      @user_friendship = UserFriendship.request users(:christina), users(:will)
    end
  ...
  context "#block!" do
    ...
    should "not allow new requests once blocked" do
      @user_friendship.block!
      uf = UserFriendship.request users(:christina), users(:will)
      assert !uf.save
    end
  end
end
